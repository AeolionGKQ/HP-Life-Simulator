import { isAndroidNative, requestPython } from "./pythonBridge";
...
export const api = {
  health: async () => {
    if (isAndroidNative()) {
      return requestPython<HealthResponse>("/api/health");
    }
    return request<HealthResponse>("/api/health");
  },
  llmConfig: () => isAndroidNative()
    ? requestPython<LLMConfigStatus>("/api/config/llm")
    : request<LLMConfigStatus>("/api/config/llm"),
  eras: () => isAndroidNative()
    ? requestPython<EraInfo[]>("/api/content/eras")
    : request<EraInfo[]>("/api/content/eras"),
  updateLlmConfig: (payload: { base_url: string; api_key: string; model: string }) =>
    isAndroidNative()
      ? requestPython<LLMConfigStatus>("/api/config/llm", "PUT", JSON.stringify(payload))
      : request<LLMConfigStatus>("/api/config/llm", {
          method: "PUT",
  ...
  deleteSession: (id: string) =>
    isAndroidNative()
      ? requestPython<void>(`/api/sessions/${id}`, "DELETE")
      : request<void>(`/api/sessions/${id}`, { method: "DELETE" }),
  session: (id: string) =>
    isAndroidNative()
      ? requestPython<SessionDetail>(`/api/sessions/${id}`)
      : request<SessionDetail>(`/api/sessions/${id}`),
  setup: (id: string) =>
    isAndroidNative()
      ? requestPython<SetupView>(`/api/sessions/${id}/setup`)
      : request<SetupView>(`/api/sessions/${id}/setup`),
  answerSetup: (id: string, step: number, answer: unknown) =>
    isAndroidNative()
      ? requestPython<SetupView>(
          `/api/sessions/${id}/setup/answer`,
          "POST",
  ...
  state: (id: string) =>
    isAndroidNative()
      ? requestPython<PlayerStateResponse>(`/api/sessions/${id}/state`)
      : request<PlayerStateResponse>(`/api/sessions/${id}/state`),
  courses: (id: string) =>
    isAndroidNative()
      ? requestPython<CourseView>(`/api/sessions/${id}/courses`)
      : request<CourseView>(`/api/sessions/${id}/courses`),
  acknowledgeDepartureNotice: (id: string) =>
    isAndroidNative()
      ? requestPython<PlayerStateResponse>(
          `/api/sessions/${id}/departure-notice/acknowledge`,
          "POST",
        )
  ...
  selectCourses: (id: string, payload: CourseSelectionRequest) =>
    isAndroidNative()
      ? requestPython<CourseView>(
          `/api/sessions/${id}/courses`,
          "PUT",
  ...
  journal: (id: string) =>
    isAndroidNative()
      ? requestPython<JournalEntry[]>(`/api/sessions/${id}/journal`)
      : request<JournalEntry[]>(`/api/sessions/${id}/journal`),
  relationships: (id: string) =>
    isAndroidNative()
      ? requestPython<Relationship[]>(`/api/sessions/${id}/relationships`)
      : request<Relationship[]>(`/api/sessions/${id}/relationships`),
  npcs: (id: string) =>
    isAndroidNative()
      ? requestPython<NPCState[]>(`/api/sessions/${id}/npcs`)
      : request<NPCState[]>(`/api/sessions/${id}/npcs`),
  turns: (id: string) =>
    isAndroidNative()
