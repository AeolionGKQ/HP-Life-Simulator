...
def _source_turns_for_new_job(db: Session, session_id: str) -> list[TurnRecord]:
    settings = get_settings()
    latest_sequence = (
        db.scalar(
            select(TurnRecord.sequence)
...
def schedule_story_arc_job(job_id: str) -> None:
    if os.environ.get("HP_SIMULATOR_ANDROID") == "1":
        threading.Thread(
            target=lambda: asyncio.run(_run_story_arc_job(job_id)),
            name=f"story-arc-{job_id[:8]}",
            daemon=True,
        ).start()
        return
